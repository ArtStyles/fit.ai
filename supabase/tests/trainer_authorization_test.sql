...
SELECT is((SELECT count(*) FROM public.trainer_plan_assignments), 0::BIGINT, 'coach_b never reads coach_a assignments');
SELECT is((SELECT count(*) FROM public.trainer_assignment_versions), 0::BIGINT, 'coach_b never reads coach_a versions');
RESET ROLE; SELECT set_config('request.jwt.claim.sub', 'a1000000-0000-4000-8000-000000000001', true); SET LOCAL ROLE authenticated;
