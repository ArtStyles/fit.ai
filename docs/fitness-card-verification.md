...
## Camera scan recovery

Reproduced two failures in the previous compiled app: a QR fully visible but outside the decoder's central crop never decoded, and a rapid camera restart lost its video stream after the previous scanner's delayed cleanup. The reader now scans the full frame with its aspect ratio preserved, caps the decoding image at 768 pixels and eight scans per second, and gives each camera session its own video element. An old session can no longer stop a replacement stream. Returning to the visible app resumes only an explicitly started, still-open reader; closing, accepting a result, choosing an image or disabling the scanner cancels that intent. Empty frames from both decoder paths are treated as normal; actual decoder failures receive visible recovery guidance.

The new live-camera regression uses real canvas MediaStreams and the production decoder. Centered, off-center and large QR codes, rapid restart, and hide/return all decode successfully and release their tracks. A sixth case exercises the native BarcodeDetector empty-result path and verifies that it keeps scanning without a false error. The off-center and restart cases failed against the previous compiled app before this fix. The complete Fitness Card browser suite passed at 320/390/1440 px and English offline, including existing sharing, permission, consent, photo and private-content cases, with no runtime errors or unexpected HTTP requests. Mobile and web TypeScript, scoped ESLint and the Android release build passed.

Signed update: `.artifacts/Vekira-1.1.24-offline.apk`, version code 26, 18,080,969 bytes, SHA-256 `bd0ab44019f40ba96fed152bc5b0bf059b96934957e0c1ff7431bc091bb075b3`. Verification passed for original signer continuity, version, alignment, exact compiled assets/index, SQLite, exercise posters, fonts, muscle-map license and absence of fixture backend settings. Install over the current app to retain data. These tests reproduce live video decoding in a controlled browser; physical-device camera behavior and installation remain unverified. No backend migration or Git push was performed for this fix.
