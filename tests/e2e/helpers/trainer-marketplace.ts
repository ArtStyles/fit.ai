...
export const TRAINER_SECURITY_PREFLIGHT_ERROR =
  'Trainer security baseline and direct-assignment migration (preflight 61) must be deployed before fixture writes'
...
export async function probeTrainerSecurityReadOnly(
  ...
  return {
    ...
    marker: markerResult.error || markerResult.data !== 61 ? null : 61,
  }
}
...
export async function assertTrainerSecuritySchemaReady(dependencies: {
  ...
  try {
    ...
    if (probe.tableError || probe.marker !== 61) throw new Error(TRAINER_SECURITY_PREFLIGHT_ERROR)
  } catch {
