...
export async function authorizeInState(state: State, clientSessionId: string, workoutId: string, now = new Date()): Promise<AuthorizeSessionStartResult> {
  ...
  // Only this branch has verified that the exact owner's session ID has no lease.
  // A stale/consumed lease or a persistence exception must never make this claim.
  const neverStarted = (error: string): AuthorizeSessionStartResult => ({ success: false, error, authorizationAbsent: true })
  const userProfile = profile(state)
  if (!['cleared', 'modified'].includes(userProfile.readiness_status)) {
    const readinessStatus = userProfile.readiness_status === 'professional_clearance_required'
      ? 'professional_clearance_required' : 'pending'
    return {
      success: false,
      error: readinessStatus === 'professional_clearance_required'
        ? 'Tu revisión de preparación requiere autorización profesional antes de entrenar.'
        : 'Completa la revisión de preparación antes de entrenar.',
      readinessStatus,
  ...
  for (const authorization of rows(state, 'session_authorizations')) {
    ...
    if (authorization.consumed_at || !authorization.released_at) return neverStarted('Ya tienes una sesión iniciada o registrada hoy. Máximo una sesión por día.')
  }
