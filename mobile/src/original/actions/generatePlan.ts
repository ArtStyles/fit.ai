// Original plan-generation flow, with its storage boundary adapted for SQLite.

import { mutate, read, type State } from './state'
import { generationClient as createClient } from './generationClient'
...
export async function findExistingPlanGeneration(
  ...
  const supabase = createClient(await read())
...
export async function generateInState(state: State, options: GeneratePlanOptions): Promise<GeneratePlanResult> {
  ...
  let exercises
  try {
    exercises = await filterExercisesForUser(state, userCtx)
  } catch (err) {
...
export async function generatePlan(options: GeneratePlanOptions): Promise<GeneratePlanResult> {
  if (options.previewOnly) return generateInState(await read(), options)
  return mutate(state => generateInState(state, options))
}
