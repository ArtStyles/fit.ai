...
import { stateClient, type State } from './state'
import type { UserContext, FilteredExercise } from '@/lib/ai/types'
...
const BODYWEIGHT_TERMS = new Set([
  ...
  'peso corporal', 'colchoneta', 'ninguno', 'suelo',
])
...
function canonicalEquipment(value: string): string | null {
  const normalized = value.toLowerCase().trim()
  if (BODYWEIGHT_TERMS.has(normalized)) return null
  if (/^mancuernas?$/.test(normalized)) return 'dumbbells'
  if (/^(barra|barra ez|barra recta|discos)$/.test(normalized)) return 'barbell'
  if (/^banco( plano| inclinado| con respaldo)?$/.test(normalized)) return 'bench'
  if (/^barra de dominadas$/.test(normalized)) return 'pull_up_bar'
  if (/^(polea.*|cuerda|agarre.*|barra de jalón)$/.test(normalized)) return 'cable_machine'
  if (/^rueda abdominal$/.test(normalized)) return 'ab_wheel'
  for (const [key, names] of Object.entries(EQUIPMENT_MAP)) {
    if (key === normalized || names.includes(normalized)) return key
  }
  return normalized
}
...
function exerciseFitsEquipment(
  ...
  const available = new Set(gymType === 'home_no_equipment' ? [] : userEquipment.map(canonicalEquipment))
  // Every required implement must be present. An empty bodyweight token must
  // never act as a substring wildcard and admit gym machines into home plans.
  return exerciseEquipment.every(value => { const required = canonicalEquipment(value); return required === null || available.has(required) })
}
...
export async function filterExercisesForUser(
  ...
  const supabase = stateClient(state)
  ...
  return filtered.map(ex => ({
    id:             ex.id,
    name:           ex.name,
    muscle_groups:  ex.muscle_groups.map(value => MUSCLE_NAMES[value.toLowerCase()] ?? value),
    equipment:      ex.equipment.flatMap(value => { const equipment = canonicalEquipment(value); return equipment === null ? [] : [equipment] }),
