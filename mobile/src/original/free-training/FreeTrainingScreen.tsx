...
import { Check, ChevronRight, Dumbbell, Plus, Search, TrendingUp } from 'lucide-react'
...
export function FreeTrainingScreen({ model, draft, draftLoadFailed = false }: { model: FreeTrainingModel; draft: FreeTrainingDraft | null; draftLoadFailed?: boolean }) {
  ...
  return <>
    ...
    <main className="mx-auto w-full max-w-lg space-y-6 px-4 pb-28 pt-6 text-foreground">
      {result ? <section className="space-y-5 rounded-2xl border border-border bg-card p-5" aria-labelledby="saved-title">
        <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-primary/10 text-primary"><Check className="h-6 w-6" /></span>
        <div><h2 id="saved-title" ref={resultHeading} tabIndex={-1} className="font-display text-2xl font-bold outline-none">{t('Entrenamiento guardado', 'Workout saved')}</h2><p className="mt-2 text-muted-foreground">{form.name || t('Entrenamiento libre', 'Free workout')} · {form.date}</p><p className="mt-2 text-sm font-medium text-primary">{detailLabel(result.detailLevel)}</p></div>
        ...
        {result.detailLevel !== 'attendance' && <div className="grid grid-cols-2 gap-3"><div className="rounded-xl bg-muted/50 p-3"><strong className="text-2xl">{result.sets}</strong><p className="text-sm text-muted-foreground">{t('Series registradas', 'Logged sets')}</p></div><div className="rounded-xl bg-muted/50 p-3"><strong className="text-2xl">{new Intl.NumberFormat(model.language, { maximumFractionDigits: 1 }).format(onlyTimedSets ? recordedSeconds : result.volumeKg)} <span className="text-sm">{onlyTimedSets ? 's' : 'kg'}</span></strong><p className="text-sm text-muted-foreground">{onlyTimedSets ? t('Tiempo registrado', 'Logged time') : t('Volumen registrado', 'Logged volume')}</p></div></div>}
        ...
        {result.improvements.map((item, i) => <div key={i} className="rounded-xl bg-primary/10 p-3 text-sm"><p className="font-semibold">{item.exerciseName}</p><p>{item.previousWeightKg} kg × {item.previousReps} → {item.weightKg} kg × {item.reps}</p></div>)}
        <div className="grid gap-2"><Button asChild className="h-12"><Link href="/progress"><TrendingUp className="mr-2 h-4 w-4" />{t('Ver mi progreso', 'View my progress')}</Link></Button><Button asChild variant="outline" className="h-12"><Link href={`/history/${result.logId}`}>{t('Ver registro', 'View entry')}</Link></Button><Button type="button" variant="outline" className="h-12" onClick={() => navigate(`/registrar?log=${result.logId}`, true)}>{t('Añadir o editar detalles', 'Add or edit details')}</Button><Button type="button" variant="ghost" className="h-12" onClick={() => navigate(`/registrar?draft=${crypto.randomUUID()}`)}>{t('Registrar otra sesión', 'Log another session')}</Button></div>
      </section> : <>
        <div className="space-y-2"><span className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1 text-xs font-medium text-primary"><Dumbbell className="h-4 w-4" />{base.current.expectedVersion == null ? t('Entrenamiento libre', 'Free workout') : t('Editar el mismo entrenamiento', 'Edit this workout')}</span><h2 className="font-display text-2xl font-bold">{t('Lo que hiciste hoy cuenta', 'What you did today counts')}</h2><p className="text-sm leading-relaxed text-muted-foreground">{t('Guarda tu entrenamiento. Si quieres, añade los ejercicios que realizaste.', 'Save your workout. If you want, add the exercises you performed.')}</p></div>
        <form className="space-y-5" onSubmit={event => { event.preventDefault(); void save() }}>
          <fieldset disabled={busy} className="min-w-0 space-y-5">
            <TrainingFields form={form} today={model.today} onChange={update} language={model.language} hasActivePlan={model.hasActivePlan} />
            <Button type="button" variant="outline" className="h-12 w-full" aria-expanded={showExercises} aria-controls="free-training-exercises" onClick={() => setShowExercises(value => !value)}><Plus className="mr-2 h-4 w-4" />{showExercises ? t('Ocultar ejercicios', 'Hide exercises') : t('Añadir ejercicios', 'Add exercises')}{form.exercises.length ? ` (${form.exercises.length})` : ''}</Button>
            {showExercises && <section id="free-training-exercises" className="space-y-4" aria-label={t('Ejercicios realizados', 'Performed exercises')}>
              ...
              <label className="block space-y-2 text-sm font-medium"><span className="inline-flex items-center gap-2"><Search className="h-4 w-4" />{t('Buscar ejercicio', 'Search exercises')}</span><Input type="search" className="h-12 text-base" placeholder={t('Nombre o músculo', 'Name or muscle')} value={search} onChange={event => setSearch(event.target.value)} /></label>
              <div className="max-h-64 space-y-1 overflow-y-auto rounded-xl border border-border p-1">{available.slice(0, 30).map(item => <button key={item.id} type="button" className="flex min-h-12 w-full items-center justify-between gap-3 rounded-lg px-3 py-2 text-left text-sm hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" onClick={() => update(addExercise(form, item))}><span className="min-w-0 break-words">{item.name}</span><Plus className="h-4 w-4 shrink-0 text-primary" /></button>)}{available.length === 0 && <p className="p-3 text-sm text-muted-foreground">{t('No hay ejercicios con esa búsqueda.', 'No exercises match your search.')}</p>}{available.length > 30 && <p className="p-3 text-xs text-muted-foreground">{t('Escribe para encontrar más ejercicios.', 'Type to find more exercises.')}</p>}</div>
            </section>}
            {form.exercises.length > 0 && <label className="flex min-h-12 cursor-pointer items-start gap-3 rounded-xl bg-muted/50 p-3 text-sm"><input type="checkbox" className="mt-1 h-5 w-5 shrink-0 accent-[hsl(var(--primary))]" checked={form.complete} onChange={e => update({ ...form, complete: e.target.checked })} /><span><span className="block font-medium">{t('Registré todo el entrenamiento', 'I logged the whole workout')}</span><span className="text-muted-foreground">{t('Si faltan ejercicios o series, lo guardaremos como parcial.', 'If exercises or sets are missing, we will save it as partial.')}</span></span></label>}
          </fieldset>
          ...
          <Button type="submit" disabled={busy} className="min-h-12 w-full whitespace-normal text-base">{busy ? t('Guardando…', 'Saving…') : base.current.expectedVersion != null ? t('Guardar cambios', 'Save changes') : form.exercises.length ? t('Guardar entrenamiento', 'Save workout') : t('Guardar constancia', 'Save attendance')}</Button>
          ...
          {draftError && <Button type="button" variant="outline" className="min-h-11 w-full" disabled={busy} onClick={() => { void persistDraft().catch(() => {}) }}>{t('Reintentar borrador', 'Retry draft')}</Button>}
        </form>
        {model.recent.length > 0 && <section className="space-y-2"><h2 className="text-sm font-semibold">{t('¿Quieres completar un registro anterior?', 'Want to complete an earlier entry?')}</h2><p className="text-xs text-muted-foreground">{t('Ábrelo para añadir detalles al mismo entrenamiento.', 'Open it to add details to the same workout.')}</p>{model.recent.filter(entry => entry.id !== model.initial.sessionId).slice(0, 3).map(entry => <Link key={entry.id} href={`/registrar?log=${entry.id}`} className="flex min-h-14 items-center justify-between gap-3 rounded-xl border border-border p-3 text-sm"><span className="min-w-0"><span className="block break-words font-medium">{entry.name}</span><span className="text-xs text-muted-foreground">{entry.date.slice(0, 10)} · {detailLabel(entry.detailLevel)}</span></span><ChevronRight className="h-4 w-4 shrink-0" /></Link>)}</section>}
      </>}
    </main>
  </>
}
