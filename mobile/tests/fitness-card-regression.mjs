...
try {
  for(const width of [320,390,1440]) {
    ...
    try {
      ...
      await editor.getByRole('combobox').focus(); await page.keyboard.press('Space')
      await expect(page.getByRole('listbox')).toBeVisible()
      await expect(page.getByRole('option',{name:'Violet · El original',exact:true})).toBeFocused()
      await page.keyboard.press('ArrowDown')
      await expect(page.getByRole('option',{name:'Ember · Fuego interior',exact:true})).toBeFocused()
      await page.keyboard.press('Enter')
      await expect(editor.getByRole('combobox')).toContainText('Ember')
