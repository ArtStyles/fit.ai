...
export default async function DeleteAccountPage() {
  ...
  return <main id="app-main-content" tabIndex={-1} className="mx-auto max-w-lg space-y-6 px-5 py-12">
    ...
    {user ? <><p className="text-sm">{copy('Cuenta', 'Account')}: <strong>{user.email}</strong></p><DeleteAccountSection /></> : <div className="space-y-4"><p>{copy('Inicia sesión para verificar qué cuenta quieres eliminar. Después abre Ajustes → Cuenta → Eliminar mi cuenta, o vuelve a esta página.', 'Sign in to verify the account you want to delete. Then open Settings → Account → Delete my account, or return to this page.')}</p><Link className="flex min-h-11 items-center justify-center rounded-lg bg-violet-600 px-4 font-semibold text-white" href="/login">{copy('Iniciar sesión para eliminar mi cuenta', 'Sign in to delete my account')}</Link><Link className="flex min-h-11 items-center justify-center rounded-lg underline" href="/recover-password">{copy('Recuperar mi contraseña', 'Recover my password')}</Link></div>}
