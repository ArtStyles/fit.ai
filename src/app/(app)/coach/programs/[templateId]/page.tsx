...
export default async function CoachProgramDetailPage({ params }: { params: { templateId: string } }) {
  ...
  if (workoutResponse.error || exerciseResponse.error) throw new Error('No se pudo cargar el editor de la rutina.')
  ...
  return <div className="min-h-screen bg-background pb-28"><PageTopBar title="Editar rutina" subtitle="Plantilla profesional" backHref="/coach/programs" backLabel="Rutinas" icon={<Dumbbell className="h-5 w-5" />} /><main className="mx-auto max-w-4xl px-4 py-8"><ProgramTemplateEditor template={template} workouts={workouts} options={(exerciseResponse.data ?? []) as PlanExerciseOption[]} /></main></div>
}
