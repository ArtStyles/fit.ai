...
export default async function CoachClientDetailPage({ params: paramsPromise, searchParams: searchParamsPromise }: {
  ...
  try {
    ...
    const assignmentResponse = relationship?.id
      ? await ((supabase as any).from('trainer_plan_assignments').select('id, status, created_at').eq('client_user_id', params.clientId).eq('trainer_user_id', user?.id).in('status', ['active', 'frozen']).order('created_at', { ascending: false }))
      : { data: null, error: null }
    if (assignmentResponse.error) throw new CoachClientStateLoadError()
    const assignments = (assignmentResponse.data ?? []) as Array<{ id: string; status: 'active' | 'frozen'; created_at: string }>
    const assignmentStatus = assignments.length === 1 ? '1 rutina asignada' : `${assignments.length} rutinas asignadas`
