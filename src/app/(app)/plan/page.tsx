...
function PlanSwitcher({ plans, tier, t }: { plans: PlanListRow[]; tier: 'free' | 'pro'; t: (source: string) => string }) {
  ...
  return (
    <details className="group mt-5 overflow-hidden rounded-2xl border border-border/60 bg-muted/10">
      <summary className="flex min-h-20 cursor-pointer list-none items-center gap-3 px-4 py-3 outline-none transition-colors hover:bg-muted/15 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-violet-500 [&::-webkit-details-marker]:hidden">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
            {activePlan ? t('Plan activo') : t('Mis planes')}
          </p>
          <p className="mt-0.5 truncate text-sm font-semibold text-foreground">
            {activePlan?.name ?? t('Crear o activar un plan')}
          </p>
          <p className="mt-0.5 text-xs text-muted-foreground">{t('Cambiar o administrar planes')}</p>
        </div>
      ...
      <div className="border-t border-border/50 bg-background/30 p-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-sm font-semibold text-foreground">{t('Planes guardados')}</p>
