...
async function loadCalendarSessionPayload(
  ...
  const { data: logs, error: logsError } = await supabase
    .from('progress_logs')
    .select(`id, workout_id, completed_at, duration_minutes, session_context_snapshot, ${process.env.NEXT_PUBLIC_LOCAL_APP === 'true' ? 'mobile_session_kind, mobile_free_training, mobile_import,' : ''} workout:workouts(name, focus)`)
    .eq('user_id', userId)
