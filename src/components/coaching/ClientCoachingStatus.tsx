...
export function ClientCoachingStatus({ requests, relationship }: { requests: ClientCoachingRequestView[]; relationship?: ClientCoachingRelationshipView }) {
  ...
  if (!requests.length && !relationship) return <p className="rounded-2xl border border-border/70 p-4 text-sm text-muted-foreground">No tienes solicitudes de acompañamiento.</p>
  ...
  return <section className="space-y-3" aria-labelledby="coaching-requests-title">
    ...
    {relationship ? <article className="rounded-2xl border border-border/70 p-4" aria-labelledby="coaching-relationship-title">
      <h3 id="coaching-relationship-title" className="font-semibold text-foreground">{relationship.status === 'active' ? 'Acompañamiento activo' : 'Acompañamiento pausado'}</h3>
    ...
    <ul className="space-y-3">{requests.map(request => <li key={request.id} className="rounded-2xl border border-border/70 p-4">
      ...
      <p className="mt-1 text-sm text-muted-foreground">Enviada el {new Intl.DateTimeFormat(dateLocale(language), { dateStyle: 'medium', timeZone }).format(new Date(request.createdAt))}</p>
