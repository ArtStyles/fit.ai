...
import type { SaveState, TemplateExerciseView } from './types'
...
export function TemplateExerciseCard({
  ...
  return (
    <li data-template-exercise-id={exercise.id} className="min-w-0 rounded-2xl border border-border/70 bg-background/60 p-3">
      ...
      {editing ? (
        <form onSubmit={event => void submit(event)} onChangeCapture={() => setSaveState('dirty')} className="mt-3 rounded-xl border border-border/60 p-3">
          <fieldset aria-label={`Editar ejercicio ${name}`} disabled={saveState === 'saving'} className="grid gap-3 sm:grid-cols-4">
            ...
            <label className="text-xs sm:col-span-2">Ejercicio<select name="exerciseId" defaultValue={exercise.exercise_id} className="mt-1 h-11 w-full rounded-lg border border-input bg-background px-2">{options.map(option => <option key={option.id} value={option.id}>{option.name}</option>)}</select></label>
            <label className="text-xs">Series<input name="sets" type="number" min="1" max="20" defaultValue={exercise.sets} className="mt-1 h-11 w-full rounded-lg border border-input bg-background px-2" /></label>
            <label className="text-xs">Repeticiones<input name="reps" type="number" min="1" max="100" defaultValue={exercise.reps} className="mt-1 h-11 w-full rounded-lg border border-input bg-background px-2" /></label>
            <label className="text-xs">Peso (kg)<input name="weightKg" type="number" min="0" max="1000" step="0.25" defaultValue={exercise.weight_kg ?? ''} className="mt-1 h-11 w-full rounded-lg border border-input bg-background px-2" /></label>
            <label className="text-xs">RPE<input name="targetRpe" type="number" min="1" max="10" step="0.5" defaultValue={exercise.target_rpe ?? ''} className="mt-1 h-11 w-full rounded-lg border border-input bg-background px-2" /></label>
            <label className="text-xs">Descanso (seg.)<input name="restSeconds" type="number" min="0" max="3600" defaultValue={exercise.rest_seconds} className="mt-1 h-11 w-full rounded-lg border border-input bg-background px-2" /></label>
            <label className="text-xs sm:col-span-3">Notas<textarea name="notes" maxLength={1000} defaultValue={exercise.notes ?? ''} rows={2} className="mt-1 min-h-11 w-full rounded-lg border border-input bg-background px-2 py-2" /></label>
