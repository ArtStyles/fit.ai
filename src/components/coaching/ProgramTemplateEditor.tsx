...
export function ProgramTemplateEditor({ template, workouts, options }: { template: ProgramTemplateView; workouts: TemplateWorkoutView[]; options: PlanExerciseOption[] }) {
  ...
  async function archive() { if (saving || !window.confirm('¿Archivar esta plantilla? Las asignaciones ya publicadas no cambiarán.')) return; setSaving(true); const data = new FormData(); data.set('templateId', template.id); try { const result: Result = await (await import('@/app/actions/trainerPrograms')).archiveTrainerProgram(data); if (result.ok) router.refresh(); setAnnouncement(result.ok ? 'Plantilla archivada.' : result.error ?? 'No se pudo archivar la plantilla.') } catch { setAnnouncement('No se pudo archivar la plantilla.') } finally { setSaving(false) } }
  return <section className="space-y-6" aria-labelledby={`program-template-${formId}`}>
    ...
    <form onSubmit={event => void saveTemplate(event)} noValidate className="rounded-2xl border border-border/70 bg-muted/10 p-4">
      ...
      <div className="mt-4 flex gap-3"><button type="submit" disabled={saving} className="min-h-11 rounded-xl bg-violet-600 px-4 text-sm font-semibold text-white disabled:opacity-50">{saving ? 'Guardando…' : 'Guardar plantilla'}</button><button type="button" disabled={saving} onClick={() => void archive()} className="min-h-11 rounded-xl border border-red-500/50 px-4 text-sm font-semibold text-red-300 disabled:opacity-50">Archivar plantilla</button></div>
    </form>
