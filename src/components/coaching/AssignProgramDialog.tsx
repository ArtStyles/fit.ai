...
export function AssignProgramDialog({
  ...
  const defaultRelationshipId = relationships.find(relationship => (
    relationship.id === selectedRelationshipId && relationship.canReceiveProposal
  ))?.id ?? relationships.find(relationship => relationship.canReceiveProposal)?.id
  const hasReadyRelationship = Boolean(defaultRelationshipId)
  ...
  return <section className="rounded-2xl border border-violet-500/30 bg-violet-500/5 p-4 [&_button[type=submit]]:bg-violet-600 [&_button[type=submit]]:text-white" aria-labelledby="assign-program-title">
    ...
    {open ? <form id="assign-program-form" onSubmit={event => void submit(event)} noValidate className="mt-4 space-y-3 rounded-xl border border-border/70 p-3">
      <fieldset disabled={busy}>
        ...
        <div className="mt-2 grid gap-2">
          {relationships.map(relationship => {
            ...
            const isBlocked = !relationship.canReceiveProposal
            return <label key={relationship.id} className={`flex items-center gap-3 rounded-xl border border-border/70 bg-background p-3 has-[:checked]:border-violet-500 has-[:checked]:bg-violet-500/5 ${isBlocked ? 'cursor-not-allowed opacity-75' : 'cursor-pointer'}`}>
              <input required type="radio" name="relationshipId" value={relationship.id} disabled={busy || isBlocked} defaultChecked={relationship.id === defaultRelationshipId} className="h-4 w-4 accent-violet-600" />
              {relationship.clientAvatarUrl ? <img src={relationship.clientAvatarUrl} alt="" className="h-10 w-10 rounded-full object-cover" /> : <span aria-hidden="true" className="flex h-10 w-10 items-center justify-center rounded-full bg-muted font-semibold">{name.slice(0, 1).toUpperCase()}</span>}
      ...
      <button type="submit" disabled={busy || !hasReadyRelationship} className="min-h-11 rounded-xl bg-primary px-4 text-sm font-semibold text-primary-foreground disabled:opacity-50">{busy ? 'Enviando…' : 'Enviar propuesta bloqueada'}</button>
    </form> : null}
