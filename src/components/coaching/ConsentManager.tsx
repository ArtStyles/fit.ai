    ...
    <article className="rounded-2xl bg-muted/30 p-4">
      ...
      {trainingGranted ? <>
        ...
        <button type="button" onClick={() => void act('grant-training')} disabled={busy !== null}
          className="mt-3 min-h-11 rounded-xl bg-primary px-4 text-sm font-semibold text-primary-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 disabled:opacity-50">{busy === 'grant-training' ? 'Autorizando…' : 'Autorizar datos de entrenamiento'}</button>
      </div>}
    </article>
    <article className="rounded-2xl bg-muted/30 p-4">
