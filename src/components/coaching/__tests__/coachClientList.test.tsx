...
describe('CoachClientList', () => {
  it('renders a non-clinical active-client card that navigates only by client id', () => {
    const html = renderToStaticMarkup(<CoachClientList clients={clients} viewerTimeZone="America/Havana" />)
  ...
  it('renders the real empty state when no active consented client is available', () => {
    const html = renderToStaticMarkup(<CoachClientList clients={[]} viewerTimeZone="America/Havana" />)
