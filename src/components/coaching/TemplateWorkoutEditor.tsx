...
export function TemplateWorkoutEditor({ workout, options }: { workout: TemplateWorkoutView; options: PlanExerciseOption[] }) {
  ...
  async function updateWorkout(event: React.FormEvent<HTMLFormElement>) { event.preventDefault(); if (saving) return; const formData = new FormData(event.currentTarget); setSaving(true); const result = await callAction(async () => (await import('@/app/actions/trainerPrograms')).updateTrainerTemplateWorkout(formData)); if (result.ok) router.refresh(); setAnnouncement(result.ok ? 'Entrenamiento actualizado.' : result.error ?? 'No se pudo actualizar el entrenamiento.'); setSaving(false) }
  async function updateExercise(event: React.FormEvent<HTMLFormElement>) { event.preventDefault(); if (saving) return; const formData = new FormData(event.currentTarget); setSaving(true); const result = await callAction(async () => (await import('@/app/actions/trainerPrograms')).updateTrainerTemplateExercise(formData)); if (result.ok) router.refresh(); setAnnouncement(result.ok ? 'Ejercicio actualizado.' : result.error ?? 'No se pudo actualizar el ejercicio.'); setSaving(false) }

  return (
    <article className="rounded-2xl border border-border/70 bg-muted/10 p-4 [&_fieldset_button]:bg-violet-600 [&_fieldset_input]:bg-background [&_fieldset_input]:text-foreground [&_fieldset_select]:bg-background [&_fieldset_select]:text-foreground [&_fieldset_textarea]:bg-background [&_fieldset_textarea]:text-foreground" aria-labelledby={`template-workout-${id}`}>
      ...
      <details className="mt-3 rounded-xl border border-border/60 p-3"><summary className="min-h-11 cursor-pointer text-sm font-semibold">Editar entrenamiento</summary><form onSubmit={event => void updateWorkout(event)} className="mt-3"><fieldset aria-label={`Editar entrenamiento ${workout.id}`} className="grid gap-3 sm:grid-cols-3"><input type="hidden" name="templateWorkoutId" value={workout.id} /><label className="text-xs">Nombre<input aria-label={`Nombre de ${workout.name}`} required name="name" maxLength={120} defaultValue={workout.name} className="mt-1 h-11 w-full rounded border px-2" /></label><label className="text-xs">Día<input aria-label={`Día de ${workout.name}`} name="dayOfWeek" type="number" min="1" max="7" defaultValue={workout.day_of_week} className="mt-1 h-11 w-full rounded border px-2" /></label><label className="text-xs">Orden<input aria-label={`Orden de ${workout.name}`} name="orderInPlan" type="number" min="1" max="7" defaultValue={workout.order_in_plan} className="mt-1 h-11 w-full rounded border px-2" /></label><button type="submit" disabled={saving} className="min-h-11 rounded bg-violet-600 px-3 text-sm font-semibold text-white disabled:opacity-50">Guardar entrenamiento</button></fieldset></form></details>
      <ol className="mt-4 space-y-2" aria-label={`Ejercicios de ${workout.name}`}>
        {workout.exercises.map((item, index) => <li key={item.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border/50 px-3 py-2 text-sm [&>details]:min-w-0 [&>details]:basis-full [&>details>summary]:flex [&>details>summary]:items-center [&_fieldset_input]:h-11 [&_fieldset_select]:h-11 [&_fieldset_textarea]:min-h-11 [&_fieldset_textarea]:py-2">
      ...
      <form onSubmit={event => void addExercise(event)} noValidate className="mt-4 grid gap-3 rounded-xl border border-violet-500/20 p-3 sm:grid-cols-2">
        ...
        <button type="submit" disabled={saving || options.length === 0} className="min-h-11 rounded-xl bg-violet-600 px-4 text-sm font-semibold text-white disabled:opacity-50">{saving ? 'Guardando…' : 'Agregar ejercicio'}</button>
      </form>
