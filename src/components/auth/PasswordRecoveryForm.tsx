...
export function PasswordRecoveryForm({ url, publicKey }: { url?: string; publicKey?: string }) {
  ...
  if (step === 'done') return <div className="space-y-5"><p role="status">{copy('Contraseña actualizada. Inicia sesión con tu nueva contraseña.', 'Password updated. Sign in with your new password.')}</p><PendingLink href="/login" className={authSubmitClassName}>{copy('Volver a iniciar sesión', 'Back to sign in')}</PendingLink></div>
  ...
  return <form onSubmit={submit} className="space-y-5">
    ...
    <PendingLink href="/login" className="flex min-h-11 items-center justify-center rounded-lg text-sm underline focus-visible:ring-2">{copy('Volver a iniciar sesión', 'Back to sign in')}</PendingLink>
  </form>
}
