...
export function AuthShell({
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-background pb-[var(--app-safe-area-bottom)]">
      <div className={`mx-auto grid w-full flex-1 items-start gap-12 px-5 py-9 sm:px-8 sm:py-12 lg:items-center lg:gap-16 ${aside ? 'max-w-6xl lg:grid-cols-[minmax(0,1fr)_minmax(0,0.9fr)]' : 'max-w-xl'}`}>
        ...
        {aside && <aside className="hidden min-w-0 rounded-3xl border border-violet-400/15 bg-gradient-to-br from-violet-500/10 via-surface-1 to-surface-1 p-8 lg:block xl:p-10">
          {aside}
        </aside>}
      </div>
    </div>
