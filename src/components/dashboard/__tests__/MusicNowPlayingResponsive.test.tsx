...
describe('MusicNowPlaying responsive composition', () => {
    ...
    async viewportWidth => {
      ...
      try {
        const geometry = await page.evaluate(() => {
          ...
          const copy = button?.previousElementSibling as HTMLElement | null
          ...
          if (!slot || !halo || !card || !cardLayer || !fallback || !button || !copy || !contentRow) {
          ...
          return {
            ...
            controlWidth: buttonBounds.width,
        ...
        expect(geometry.copyRight).toBeLessThanOrEqual(geometry.buttonLeft)
  ...
  it('invokes only the confirmed dynamic action and never toggles the label optimistically', async () => {
    ...
    try {
      ...
      await pause.click()
      expect(await page.evaluate(() => window.__musicControlCalls)).toEqual({ play: 1, pause: 1 })
      await pwExpect(pause).toBeVisible()
