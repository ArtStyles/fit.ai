  ...
  return (
    <SettingsSection title={title}>
      <nav aria-label={title} className="space-y-3">
        {entries.map(({ href, label, description, icon }) => (
          <PendingLink
            key={href}
            href={href}
            showSpinner={false}
          ...
          >
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-violet-500/10 text-violet-300">
              <PendingLinkIcon name={icon} className="h-4 w-4" />
            </span>
            <span className="min-w-0 flex-1">
              <span className="block text-sm font-medium text-foreground">{label}</span>
              <span className="mt-0.5 block text-xs leading-5 text-muted-foreground">{description}</span>
            </span>
            <PendingLinkIcon name="chevron-right" className="h-4 w-4 shrink-0 text-muted-foreground" />
          </PendingLink>
        ))}
      </nav>
    </SettingsSection>
